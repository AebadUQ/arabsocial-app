...
// NOTE: path aapke project ke mutabiq
...
export type CommentItemType = {
  ...
  // ⬇️ NEW: replies count from backend
...
const CommentsSheet = forwardRef<CommentsSheetHandle, Props>(
  ({ loadPage, pageSize = 10, title = "Comments" }, ref) => {
    ...
    // Handlers for actions under each comment (wire to your reply/replies API as needed)
    const onReplyPress = (c: CommentItemType) => {
      // open inline reply input or a reply sheet — your choice
      // console.log("Reply to:", c.id);
    };
    const onViewMoreReplies = (c: CommentItemType) => {
      // load replies for c.id
      // console.log("View replies for:", c.id);
    };
      ...
      return (
        <View style={[styles.commentRow, { borderBottomColor: theme.colors.borderColor }]}>
          ...
          <View style={styles.commentBody}>
            ...
            {/* Actions row: Reply + View more replies (conditional) */}
            <View style={styles.commentActions}>
              <TouchableOpacity onPress={() => onReplyPress(item)}>
                <Text style={[styles.actionText, { color: theme.colors.primary }]}>Reply</Text>
              </TouchableOpacity>

              {repliesCount > 0 && (
                <>
                  <Text style={[styles.dot, { color: theme.colors.textLight }]}>•</Text>
                  <TouchableOpacity onPress={() => onViewMoreReplies(item)}>
